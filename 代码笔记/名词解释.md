...
命令行界面。**CLI** 是 **"命令行界面"**（Command Line Interface）的缩写。它是一种通过文本命令与计算机系统或软件交互的用户界面，用户可以通过输入特定的命令来执行各种操作，如文件管理和程序运行。

### softmax

在数学，尤其是概率论和相关领域中，**Softmax函数**，或称**归一化指数函数**，是[逻辑斯谛函数](https://zh.wikipedia.org/wiki/逻辑斯谛函数)的一种推广。它能将一个含任意实数的K维向量 z![{\displaystyle \mathbf {z} }](https://wikimedia.org/api/rest_v1/media/math/render/svg/82eca5d0928078d5a61b9e7e98cc73db31070909) “压缩”到另一个K维实向量 σ(z)中，使得每一个元素的范围都在(0,1)之间，并且所有元素的和为1(也可视为一个 (k-1)维的hyperplane或subspace)。该函数的形式通常按下面的式子给出：
$$
\sigma(\mathbf{z})_{j} = \frac{e^{z_{j}}}{\sum_{k=1}^{K} e^{z_{k}}} \quad \text{for } j = 1, \cdots, K.
$$
...
| 特性         | BF16 (Brain Floating Point 16) | FP16 (Half Precision) |
| ------------ | ------------------------------ | --------------------- |
| **开发者**   | Google Brain (2018)            | IEEE 754标准 (2008)   |
| **指数位**   | 8位                            | 5位                   |
| **尾数位**   | 7位                            | 10位                  |
| **数值范围** | ±3.4e38 (大)                   | ±65504 (小)           |
| **精度**     | 较低 (约1-2%相对误差)          | 较高 (约0.1%相对误差) |
| **内存占用** | 2字节                          | 2字节                 |
| **硬件支持** | 现代AI加速器 (Ampere+)         | 广泛支持              |

### cuda-graph-max-bs

    "含义": "CUDA Graph捕获时尝试的最大批次大小",
    "CUDA Graph": {
        "目的": "将计算图预编译，避免每次推理重新调度",
        "优势": "提升推理速度2-5倍",
        "代价": "需要额外显存存储编译后的图"
    },
    "内存需求": {
        "公式": "额外内存 ≈ 批大小 × 序列长度 × 隐藏维度 × 数据类型大小",
        "示例": "bs=8, seq_len=4096, hidden=4096, bf16 → 约256MB",
        "批大小影响": "bs越大，图越大，内存需求越高"
    }

### SIMD

“Single Instruction, Multiple Data”的缩写，意思是“单指令多数据”。它是一种并行计算架构，允许单条指令同时对多个数据项进行操作，尤其适用于处理大量重复数据的操作。在推理引擎中，利用 SIMD 整数指令集可以高效地完成矩阵乘法等核心运算，充分利用 CPU/GPU/NPU 的并行处理能力，从而提高计算性能。
